...
public final class EagerBinaryPrimitiveNodeWithArgumentRead extends EagerPrimitive {

    @Child private ExpressionNode receiver;
    @Child private ArgumentReadNode.LocalArgumentReadNode argument;
    @Child private BinaryExpressionNode primitive;

    private final SSymbol selector;

    public EagerBinaryPrimitiveNodeWithArgumentRead(final SourceSection source,
                                    final SSymbol selector,
                                    final ExpressionNode receiver,
                                    final ArgumentReadNode.LocalArgumentReadNode argument,
    ...
    protected boolean isTaggedWith(final Class<?> tag) {
        assert !(primitive instanceof InstrumentableFactory.WrapperNode) : "primitive can't be WrapperNodes to avoid double wrapping. It is: " + primitive.getClass().getSimpleName() + " and contains a " + ((InstrumentableFactory.WrapperNode) primitive).getDelegateNode().getClass().getSimpleName();
    ...
    private MessageSendNode.GenericMessageSendNode makeGenericSend() {
        VM.insertInstrumentationWrapper(this);

        MessageSendNode.GenericMessageSendNode node = MessageSendNode.createGeneric(selector,
    ...
    protected void onReplace(final Node newNode, final CharSequence reason) {
        ...
        } else if (newNode instanceof InstrumentableFactory.WrapperNode) {
            assert ((InstrumentableFactory.WrapperNode) newNode).getDelegateNode() == this : "Wrapping should not also do specialization or other changes, I think";
        } else {
